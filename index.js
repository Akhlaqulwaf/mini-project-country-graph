...
const contentName = document.querySelector(".name1");
const contentBar = document.querySelector(".bar1");
const contentResult = document.querySelector(".result1");
const contentBar2 = document.querySelector(".bar2");
const contentResult2 = document.querySelector(".result2");
...
const totalLeng = countries_data.map((el) => el.name).length;
textTotal.textContent = `Currently, we have ${totalLeng} countries`;

const fetchLanguage = (viewNumber) => {
  const views = document.querySelectorAll(
    ".country-container1, .country-container2"
  );
  views.forEach((view) => (view.style.display = "none"));

  const viewToShow = document.querySelector(`.country-container${viewNumber}`);
  if (viewToShow) {
    viewToShow.style.display = "block";
    most.textContent = "10 Most Spoken Languages In The World";
    viewToShow.innerHTML = "";
    ...
    const sorted = Object.entries(language)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 10);
    for (let i = 0; i < sorted.length; i++) {
      const data = sorted[i];

      viewToShow.insertAdjacentHTML(
        "beforeend",
        ` <div class ="country">
          <p class="country-name">
          ${data[0]}
          </p>
          <p class="country-graph"><span class="graph" style="width:${data[1]}%">&nbsp;</span>
          </p>
...
const fetchPopulation = (viewNumber) => {
  const views = document.querySelectorAll(
    ".country-container1, .country-container2"
  );
  views.forEach((view) => (view.style.display = "none"));
  ...
  if (viewToShow) {
    ...
    const sorted = countries_data
      ...
      .slice(0, 10);
    const arr = sorted.map((el) => [el.name, el.population]);
